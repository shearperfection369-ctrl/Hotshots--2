...
- NEW: Backhaul Matcher (/api/backhaul/matches + Backhaul Radar panel on Brokerage Hunter tab) — matches trucks out delivering + Carrier Network standing deadhead lanes against board loads (perfect / reposition / standing). First Strike prioritizes backhaul lanes and badges bids "backhaul".
- NEW: Rate Card Utilization — bookings store rate_card_id; each card shows moved-this-week vs committed capacity with progress bar; banner totals.
- NEW: Carrier Outreach Emails — POST /api/carrier-network/outreach/{pid}: branded pitch email per category with carrier packet PDF attached (Resend when key present; records with status recorded_no_key otherwise), auto-advances prospect target→contacted. Send Pitch UI in prospect detail dialog.
- Testing: iteration_82.json — frontend 100% pass, no issues.
- NEW: Lane Stacking (/api/lane-stacking/* + panel on Hunter tab) — detects lanes with 3+ open board loads, chains them with a 2–3% carrier predictability discount kept as margin; one-click Book Stack creates 3 tagged bookings (uses contract carrier/rate card when a card covers the lane); booked stacks list totals discount kept. lane_stacks added to wipe categories.
