...
# Lets an admin re-skin the entire TMS to a different company by typing a name.
# The Tennant "default" brand is hard-coded below. Any custom brand created at
# runtime is generated by Claude Sonnet (Emergent universal key) and stored in
# Mongo so the same brand sticks across deploys.
...
async def branding_active(_: User = Depends(get_current_user)):
    """Returns the currently active company brand. Falls back to the
    hard-coded Tennant profile if nothing has been generated yet. Frontend
    fetches this once at boot and uses it to recolor / rebrand the entire UI."""
...
@api_router.get("/branding/all")
async def branding_list(_: User = Depends(require_role("admin"))):
    """List every brand the admin has generated. Default Tennant is implicit."""
...
@api_router.post("/branding/generate")
async def branding_generate(payload: BrandGenerateIn, user: User = Depends(require_role("admin"))):
    """Generate a full company brand from just a name. Uses Claude Sonnet to
    research likely brand identity (industry, color palette, product portfolio,
    supplier examples) and persists the result. If activate=True the new brand
    becomes the active theme for every user immediately on next page load."""
    ...
    # Parse the JSON the LLM returned (be lenient — strip code fences if any).
    raw = (reply or "").strip()
    if raw.startswith("```"):
        raw = re.sub(r"^```[a-zA-Z]*\n?|\n?```$", "", raw).strip()
    try:
        ...
        # If parsing fails, salvage by trying to find the first {...} block
    ...
    # Validate / normalize
    required = ["company_name", "short_name", "primary_color"]
    for f in required:
    ...
    # Upsert so re-generating the same brand_id replaces the old one.
...
async def branding_delete(brand_id: str, _: User = Depends(require_role("admin"))):
